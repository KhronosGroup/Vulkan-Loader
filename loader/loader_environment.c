...
VkResult loader_add_environment_layers(struct loader_instance *inst, const enum layer_type_flags type_flags,
    ...
    for (uint32_t cur_layer = 0; cur_layer < inst->settings->layer_settings.forced_on_layers_count; ++cur_layer) {
        ...
        for (uint32_t i = 0; i < source_list->count; i++) {
            ...
            if (0 == strcmp(inst->settings->layer_settings.forced_on_layers[cur_layer], source_prop->info.layerName)) {
                found = true;
                // Make sure we haven't added it already. This is a separate check in case the same layer name appears multiple
                // times in VK_INSTANCE_LAYERS - should only enable one layer
                if (!loader_find_layer_name_in_list(source_prop->info.layerName, target_list)) {
                    ...
                    if (0 == (source_prop->type_flags & VK_LAYER_TYPE_FLAG_META_LAYER)) {
                        ...
                        if (res == VK_ERROR_OUT_OF_HOST_MEMORY) return res;
                        res = loader_add_layer_properties_to_list(inst, expanded_target_list, 1, source_prop);
                        if (res == VK_ERROR_OUT_OF_HOST_MEMORY) return res;
                    } else {
                        res = loader_add_meta_layer(inst, inst->settings, source_prop, target_list, expanded_target_list,
                                                    source_list, NULL);
                        if (res == VK_ERROR_OUT_OF_HOST_MEMORY) return res;
                    }
                }
...
VkResult read_env_var_path(const struct loader_instance *inst, uint32_t log_msg_flag, char **output_var, char *env_var) {
...
VkResult parse_env_loader_log_settings(struct loader_instance *inst, struct loader_log_settings *settings) {
    char *env = NULL;
    char *orig = NULL;
    VkResult res = VK_SUCCESS;
...
VkResult parse_env_instance_settings(struct loader_instance *inst, struct loader_instance_settings *instance_settings) {
    char *env = NULL;
...
VkResult parse_env_layer_settings(struct loader_instance *inst, struct loader_layer_settings *layer_settings) {
    VkResult res = VK_SUCCESS;
    char *env = NULL;
...
VkResult parse_env_driver_settings(struct loader_instance *inst, struct loader_driver_settings *driver_settings) {
    VkResult res = VK_SUCCESS;
...
VkResult parse_env_physical_device_settings(struct loader_instance *inst,
...
void free_list(const struct loader_instance *inst, uint32_t *list_count, char ***list_ptr) {
...
// Free the paths we used for setting up the search paths for layers and drivers.
void free_search_paths(const struct loader_instance *inst, char **driver_path, char **implicit_path, char **explicit_path,
                       char **settings_path) {
    free_search_path(inst, driver_path);
    free_search_path(inst, implicit_path);
    free_search_path(inst, explicit_path);
    free_search_path(inst, settings_path);
}

// Figure out the default search paths for driver and layers based on the current operating
// system.
VkResult loader_default_search_paths(const struct loader_instance *inst, char **default_driver_path, char **default_implicit_path,
...
out:
    if (VK_SUCCESS != result) {
        free_search_paths(inst, default_driver_path, default_implicit_path, default_explicit_path, default_settings_path);
    }
...
void log_loader_settings(struct loader_instance *inst, struct loader_settings *settings) {
    if ((settings->log_settings.enabled_log_flags & VULKAN_LOADER_SETTING_BIT) != 0) {
        ...
        if (settings->driver_search_paths != NULL) {
            for (uint32_t path = 0; path < settings->driver_search_paths_count; ++path) {
                loader_log(inst, VULKAN_LOADER_SETTING_BIT, 0, "      %s", settings->driver_search_paths[path]);
            }
        } else {
        ...
        if (settings->driver_settings.select_filters != NULL) {
            ...
            for (uint32_t filt = 0; filt < settings->driver_settings.select_filters->count; ++filt) {
                loader_log(inst, VULKAN_LOADER_SETTING_BIT, 0, "      %s",
        ...
        if (settings->physical_device_settings.device_select_enabled &&
            ...
            loader_log(inst, VULKAN_LOADER_SETTING_BIT, 0, "    Device Select: %s",
        ...
        if (settings->implicit_layer_search_paths != NULL) {
            for (uint32_t path = 0; path < settings->implicit_layer_search_paths_count; ++path) {
                loader_log(inst, VULKAN_LOADER_SETTING_BIT, 0, "      %s", settings->implicit_layer_search_paths[path]);
            }
        }
        ...
        if (settings->explicit_layer_search_paths != NULL) {
            for (uint32_t path = 0; path < settings->explicit_layer_search_paths_count; ++path) {
                loader_log(inst, VULKAN_LOADER_SETTING_BIT, 0, "      %s", settings->explicit_layer_search_paths[path]);
            }
        }
        ...
        loader_log(inst, VULKAN_LOADER_SETTING_BIT, 0, "    Forced On Layers: %d", settings->layer_settings.forced_on_layers_count);
        if (settings->layer_settings.forced_on_layers != NULL) {
            for (uint32_t layer = 0; layer < settings->layer_settings.forced_on_layers_count; ++layer) {
                loader_log(inst, VULKAN_LOADER_SETTING_BIT, 0, "      %s", settings->layer_settings.forced_on_layers[layer]);
            }
        }
...
void free_settings_struct(struct loader_instance *inst, struct loader_settings **set_struct) {
    ...
    if (NULL != settings->layer_settings.forced_on_layers) {
        free_list(inst, &settings->layer_settings.forced_on_layers_count, &settings->layer_settings.forced_on_layers);
    }
    ...
    if (NULL != settings->driver_search_paths) {
        free_list(inst, &settings->driver_search_paths_count, &settings->driver_search_paths);
    }
    if (NULL != settings->implicit_layer_search_paths) {
        free_list(inst, &settings->implicit_layer_search_paths_count, &settings->implicit_layer_search_paths);
    }
    if (NULL != settings->explicit_layer_search_paths) {
        free_list(inst, &settings->explicit_layer_search_paths_count, &settings->explicit_layer_search_paths);
    }
...
VkResult generate_settings_struct(struct loader_instance *inst, struct loader_settings **set_struct) {
    ...
    *set_struct = settings;
    ...
    result = loader_default_search_paths(inst, &default_driver_search_path, &default_implicit_layer_search_path,
    ...
    result = generate_complete_search_path(inst, LOADER_DATA_FILE_SETTINGS_FILE, default_settings_file_search_path, NULL, NULL,
    ...
    result = parse_settings_file(inst, settings, settings_file_search_paths_count, settings_file_search_paths);
    if (VK_SUCCESS != result) {
        goto out;
    }

    // Override any settings with environment variable versions
    result = parse_env_loader_log_settings(inst, &settings->log_settings);
    if (VK_SUCCESS != result) {
        goto out;
    }
    result = parse_env_instance_settings(inst, &settings->instance_settings);
    if (VK_SUCCESS != result) {
        goto out;
    }
    result = parse_env_layer_settings(inst, &settings->layer_settings);
    if (VK_SUCCESS != result) {
        goto out;
    }
    result = parse_env_driver_settings(inst, &settings->driver_settings);
    if (VK_SUCCESS != result) {
        goto out;
    }
    result = parse_env_physical_device_settings(inst, &settings->physical_device_settings);
    if (VK_SUCCESS != result) {
        goto out;
    }

    // Path settings
    result = read_env_var_path(inst, VULKAN_LOADER_DRIVER_BIT, &override_driver_env_var_search_path, VK_DRIVER_FILES_ENV_VAR);
    if (VK_SUCCESS != result) {
        result = read_env_var_path(inst, VULKAN_LOADER_DRIVER_BIT, &override_driver_env_var_search_path, VK_ICD_FILENAMES_ENV_VAR);
        if (VK_SUCCESS != result) {
            goto out;
        }
    }
    result = read_env_var_path(inst, VULKAN_LOADER_DRIVER_BIT, &additional_driver_env_var_search_path,
                               VK_ADDITIONAL_DRIVER_FILES_ENV_VAR);
    if (VK_SUCCESS != result) {
        goto out;
    }
    result = read_env_var_path(inst, VULKAN_LOADER_LAYER_BIT, &override_explicit_layer_env_var_search_path, VK_LAYER_PATH_ENV_VAR);
    if (VK_SUCCESS != result) {
        goto out;
    }
    result = read_env_var_path(inst, VULKAN_LOADER_LAYER_BIT, &additional_explicit_layer_env_var_search_path,
                               VK_ADDITIONAL_LAYER_PATH_ENV_VAR);
    if (VK_SUCCESS != result) {
        goto out;
    }
    result = generate_complete_search_path(
    ...
    result =
        generate_complete_search_path(inst, LOADER_DATA_FILE_MANIFEST_IMPLICIT_LAYER, default_implicit_layer_search_path, NULL,
    ...
    log_loader_settings(inst, settings);

out:
    if (settings_file_search_paths != NULL) {
        free_list(inst, &settings_file_search_paths_count, &settings_file_search_paths);
    }
    free_search_paths(inst, &default_driver_search_path, &default_implicit_layer_search_path, &default_explicit_layer_search_path,
                      &default_settings_file_search_path);
    free_search_paths(inst, &override_driver_env_var_search_path, &override_explicit_layer_env_var_search_path,
                      &additional_driver_env_var_search_path, &additional_explicit_layer_env_var_search_path);
    if (result != VK_SUCCESS) {
        free_settings_struct(inst, &settings);
        *set_struct = NULL;
    }
    return result;
}
